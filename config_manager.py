...
class ConfigManager:
    ...
    def create_api_tab(self):
        ...
        # Thông số tối ưu AI
        row_ai = 2
        tk.Label(config_frame, text="Requests per minute (Gemini RPM):").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.rpm_entry = tk.Entry(config_frame, width=20)
        self.rpm_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        tk.Label(config_frame, text="Batch delay (giây):").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.batch_delay_entry = tk.Entry(config_frame, width=20)
        self.batch_delay_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        tk.Label(config_frame, text="Concurrent requests:").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.concurrent_entry = tk.Entry(config_frame, width=20)
        self.concurrent_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        tk.Label(config_frame, text="Images per request:").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.imagesper_entry = tk.Entry(config_frame, width=20)
        self.imagesper_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        tk.Label(config_frame, text="Min delay giữa requests (giây):").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.mindelay_entry = tk.Entry(config_frame, width=20)
        self.mindelay_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        tk.Label(config_frame, text="Max image size (px, cạnh lớn nhất):").grid(row=row_ai, column=0, padx=5, pady=5, sticky="w")
        self.maximg_entry = tk.Entry(config_frame, width=20)
        self.maximg_entry.grid(row=row_ai, column=1, padx=5, pady=5, sticky="w")

        row_ai+=1
        self.downscale_var = tk.BooleanVar()
        self.downscale_check = tk.Checkbutton(config_frame, text="Downscale ảnh trước khi gửi", variable=self.downscale_var)
        self.downscale_check.grid(row=row_ai, column=0, columnspan=2, sticky="w", pady=(0,5))

        # Khi load cấu hình sẽ tự điền các trường này
        # Khi lưu sẽ thêm các trường này vào json
    ...
    def load_api_config(self):
        ...
        try:
            with open("api_config.json", "r", encoding="utf-8") as f:
                ...
                self.rpm_entry.insert(0, str(config.get("GEMINI_RPM_LIMIT", 15)))
                self.batch_delay_entry.delete(0, tk.END)
                self.batch_delay_entry.insert(0, str(config.get("BATCH_DELAY", 1.5)))
                self.concurrent_entry.delete(0, tk.END)
                self.concurrent_entry.insert(0, str(config.get("CONCURRENT_REQUESTS", 3)))
                self.imagesper_entry.delete(0, tk.END)
                self.imagesper_entry.insert(0, str(config.get("IMAGES_PER_REQUEST", 5)))
                self.mindelay_entry.delete(0, tk.END)
                self.mindelay_entry.insert(0, str(config.get("MIN_DELAY", 0.5)))
                self.maximg_entry.delete(0, tk.END)
                self.maximg_entry.insert(0, str(config.get("MAX_IMAGE_SIZE", 1600)))
                self.downscale_var.set(bool(config.get("ENABLE_DOWNSCALE", True)))
        except FileNotFoundError:
            pass
    ...
    def save_api_config(self):
        ...
        # Lấy thông số tối ưu
        try:
            rpm = int(self.rpm_entry.get().strip())
        except:
            rpm = 15
        try:
            bdelay = float(self.batch_delay_entry.get().strip())
        except:
            bdelay = 1.5
        try:
            conc = int(self.concurrent_entry.get().strip())
        except:
            conc = 3
        try:
            imgper = int(self.imagesper_entry.get().strip())
        except:
            imgper = 5
        try:
            mindelay = float(self.mindelay_entry.get().strip())
        except:
            mindelay = 0.5
        try:
            maximg = int(self.maximg_entry.get().strip())
        except:
            maximg = 1600
        enable_downscale = self.downscale_var.get()
        ...
        config = {
            ...
            "IMAGES_PER_REQUEST": imgper,
            "MIN_DELAY": mindelay,
            "MAX_IMAGE_SIZE": maximg,
            "ENABLE_DOWNSCALE": enable_downscale
        }
        with open("api_config.json", "w", encoding="utf-8") as f:
            json.dump(config, f, indent=2, ensure_ascii=False)
        self.api_status_label.config(text="✅ Đã lưu cấu hình API/AI", fg="green")
        messagebox.showinfo("Thành công", "Đã lưu cấu hình AI và Gemini!")
